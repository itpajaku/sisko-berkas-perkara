...
if (!$berkas->tanggal_pbt && !$berkas->tanggal_bht) { ?>
    ...
    <div
        ...
        class="badge rounded-pill  me-2 bg-danger text-white d-flex align-items-center"
...
<?php } else if (!$berkas->tanggal_bht) { ?>
    ...
    <div
        id="kolom-selisih-<?= App\Libraries\Hashid::encode($berkas->id) ?>"
        class="badge rounded-pill bg-warning me-2 text-white d-flex align-items-center"
...
<?php } else { ?>
    <?php if ($berkas->status == 0) { ?>
        <div
            id="kolom-selisih-<?= App\Libraries\Hashid::encode($berkas->id) ?>"
            class="badge rounded-pill bg-primary me-2 text-white d-flex align-items-center"
    ...
    <?php } else { ?>
        <div
            id="kolom-selisih-<?= App\Libraries\Hashid::encode($berkas->id) ?>"
            class="badge rounded-pill me-2 bg-success-subtle text-success d-flex align-items-center"
