<div class="container-fluid">
  ...
  <div class="card mb-4">
    <div class="card-body">
      ...
      <canvas id="chart-berkas-harian" height="80" style="display:none"></canvas>
    </div>
  </div>
  <div
    class="card"
    hx-get="<?= base_url('dashboard_gugatan/total_berkas') ?>"
    hx-trigger="load delay:1s"
    hx-swap="innerHTML"
    hx-indicator="#loading-indicator"
    hx-headers='{"HX-Request-Component":"true"}'>
    <div class="text-center p-3 bg-primary bg-opacity-10" id="loading-indicator">
      Memuat Konten. Mohon Tunggu ...
    </div>
  </div>
  <div class="row"
    hx-get="<?= base_url('dashboard_gugatan/total_ekspedisi_berkas') ?>"
  ...
  <script>
    ...
    function loadChartBerkas(bulan, tahun) {
      const url = `<?= base_url('dashboard_gugatan/chart_berkas_harian') ?>?bulan=${bulan}&tahun=${tahun}`;
      fetch(url)
        .then(res => res.json())
        .then(json => {
          const options = {
